...
module.exports = function(targetEnv, docClient) {
    ...
    return {
        ...
        // Return the list of Company identifiers that are associated with the 'AVC' CompanyProfile
        listAVCCompanies: function() {
        ...
        // Migrate Flags for a given 'UserAndCompanyUid' so that it gets compliant with the AVC offer
        //
        ...
        migrateFor: function(userAndCompanyFlagsIdentifier) {
            return new BPromise(function(resolve, reject) {
                docClient.get({
                    TableName: targetEnv + "_AV_UI_UserFlags",
                    Key: {
                    ...
                    if (err) {
                        ...
                        if (!data.Item.prefVersion || data.Item.prefVersion.indexOf("16.0") === -1) {
                            ...
                            if (data.Item.startDashboard_widgetsOrder) {
                                console.log("  - A 'startDashboard_widgetsOrder' configuration is available. It will be removed!");
                                attributeUpdates.startDashboard_widgetsOrder = {
                                    Action: "DELETE"
                                };
                            }
                            if (data.Item.monitorSystemsDashboard_widgetsOrder) {
                                console.log("  - A 'monitorSystemsDashboard_widgetsOrder' configuration is available. It will be removed!");
                                attributeUpdates.monitorSystemsDashboard_widgetsOrder = {
                                    Action: "DELETE"
                                };
                            }
                            if (data.Item.MON_SYS_DET_PREF_KEY) {
                                console.log("  - A 'MON_SYS_DET_PREF_KEY' configuration is available. It will be removed!");
                            ...
                            if (data.Item.startDashboard_widgetsOrder && data.Item.startDashboard_widgetsOrder.indexOf("\"widgetType\":\"fleetdata\"") !== -1) {
                                ...
                                attributeUpdates.monitorSystemsDashboard_widgetsOrder_oldOffer = {
                                    Action: "PUT",
